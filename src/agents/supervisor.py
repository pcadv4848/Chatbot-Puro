...
async def _processar_silencioso(texto: str, sessao: SessionState) -> str:
    """Modo silencioso: salva sessão e retorna SILENT sem processar nada."""
...
async def processar(texto: str, sessao: SessionState) -> str:
    ...
    if sessao.human_attending:
        return await _processar_silencioso(texto, sessao)
...
async def _processar_gerando(sessao: SessionState, force: bool = False) -> str:
    ...
    await salvar_sessao(sessao)
    return "Seus dados foram registrados com sucesso! Em breve um advogado entrará em contato para dar continuidade ao seu caso."
